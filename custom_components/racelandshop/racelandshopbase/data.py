...
class RacelandshopData:
    """RacelandshopData class."""
    ...
    async def async_write(self):
        ...
        if self.racelandshop.status.background_task or self.racelandshop.system.disabled:
            return
        ...
        await async_save_to_store(
            self.racelandshop.hass,
            "racelandshop",
            {
                "view": self.racelandshop.configuration.frontend_mode,
                "compact": self.racelandshop.configuration.frontend_compact,
                "onboarding_done": self.racelandshop.configuration.onboarding_done,
            },
        )
        await self._async_store_content_and_repos()
        for event in ("racelandshop/repository", "racelandshop/config"):
            self.racelandshop.hass.bus.async_fire(event, {})
    ...
    async def _async_store_content_and_repos(self):  # bb: ignore
        ...
        for repository in self.racelandshop.repositories:
            await self.async_store_repository_data(repository)

        await async_save_to_store(self.racelandshop.hass, "repositories", self.content)
    ...
    async def async_store_repository_data(self, repository):
        ...
        if (
            ...
            await async_save_to_store_default_encoder(
                self.racelandshop.hass,
                f"racelandshop/{repository.data.id}.racelandshop",
                export,
            )
    ...
    async def restore(self):
        """Restore saved data."""
        racelandshop = await async_load_from_store(self.racelandshop.hass, "racelandshop")
        repositories = await async_load_from_store(self.racelandshop.hass, "repositories") or {}

        if not racelandshop and not repositories:
            # Assume new install
            self.racelandshop.status.new = True
            return True
        self.logger.info("Restore started")
        self.racelandshop.status.new = False

        # Racelandshop
        self.racelandshop.configuration.frontend_mode = racelandshop.get("view", "Grid")
        self.racelandshop.configuration.frontend_compact = racelandshop.get("compact", False)
        self.racelandshop.configuration.onboarding_done = racelandshop.get("onboarding_done", False)

        # Repositories
        hass = self.racelandshop.hass
        stores = {}
        ...
        try:
            ...
            for entry, repo_data in repositories.items():
                if self.async_restore_repository(entry, repo_data):
                    stores[entry] = get_store_for_key(hass, f"racelandshop/{entry}.racelandshop")

            def _load_from_storage():
                for entry, store in stores.items():
                    if os.path.exists(store.path) and (data := store.load()):
                        update_repository_from_storage(self.racelandshop.get_by_id(entry), data)
    ...
    async def register_unknown_repositories(self, repositories):
        ...
        register_tasks = [
            ...
            for entry, repo_data in repositories.items()
            if not self.racelandshop.is_known(entry)
        ]
        if register_tasks:
    ...
    def async_restore_repository(self, entry, repository_data):
        ...
        if not (repository := self.racelandshop.get_by_name(full_name)):
            self.logger.error(f"Did not find {full_name} ({entry})")
            return False
        # Restore repository attributes
        self.racelandshop.async_set_repository_id(repository, entry)
        ...
        if repository_data["full_name"] == "racelandshop/integration":
