...
from custom_components.racelandshop.helpers import RepositoryHelpers
...
from custom_components.racelandshop.helpers.classes.repositorydata import RepositoryData
from custom_components.racelandshop.helpers.classes.validate import Validate
from custom_components.racelandshop.helpers.functions.download import async_download_file
...
from custom_components.racelandshop.helpers.functions.save import async_save_file
from custom_components.racelandshop.helpers.functions.store import async_remove_store
from custom_components.racelandshop.helpers.functions.validate_repository import (
    common_update_data,
    common_validate,
)
...
class RacelandshopRepository(RepositoryHelpers):
    """RacelandshopRepository."""

    def __init__(self):
        """Set up RacelandshopRepository."""
    ...
    async def common_registration(self):
        ...
        if self.repository_object is None:
            try:
                self.repository_object, etag = await get_repository(
                    self.racelandshop.session,
                    self.racelandshop.configuration.token,
                    self.data.full_name,
        ...
        if self.racelandshop.system.action:
            if self.data.description is None or len(self.data.description) == 0:
                raise RacelandshopException("::error:: Missing repository description")
    ...
    async def async_download_zip_file(self, content, validate):
        ...
        try:
            ...
            temp_dir = await self.racelandshop.hass.async_add_executor_job(tempfile.mkdtemp)
            ...
            if result:
                ...
                await self.racelandshop.hass.async_add_executor_job(cleanup_temp_dir)
                return
    ...
    def remove(self):
        ...
        if self.data.id in self.racelandshop.common.installed:
            self.racelandshop.common.installed.remove(self.data.id)
        for repository in self.racelandshop.repositories:
            if repository.data.id == self.data.id:
                self.racelandshop.async_remove_repository(repository)

    async def uninstall(self):
        """Run uninstall tasks."""
        self.logger.info("%s Uninstalling", self)
        if not await self.remove_local_directory():
            raise RacelandshopException("Could not uninstall")
        ...
        if self.data.category == "integration":
            ...
            try:
                ...
                pass
        if self.data.full_name in self.racelandshop.common.installed:
            self.racelandshop.common.installed.remove(self.data.full_name)

        await async_remove_store(self.racelandshop.hass, f"racelandshop/{self.data.id}.racelandshop")
    ...
    async def remove_local_directory(self):
        ...
        try:
            if self.data.category == "python_script":
                ...
                if os.path.exists(
                    f"{self.racelandshop.core.config_path}/{self.racelandshop.configuration.theme_path}/{self.data.name}.yaml"
                ):
                    os.remove(
                        f"{self.racelandshop.core.config_path}/{self.racelandshop.configuration.theme_path}/{self.data.name}.yaml"
                    )
