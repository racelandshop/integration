...
from custom_components.racelandshop.const import VERSION_STORAGE
from .logger import getLogger

_LOGGER = getLogger()


class RACELANDSHOPStore(Store):
...
def get_store_key(key):
    ...
    return key if "/" in key else f"racelandshop.{key}"


def _get_store_for_key(hass, key, encoder):
    """Create a Store object for the key."""
    return RACELANDSHOPStore(hass, VERSION_STORAGE, get_store_key(key), encoder=encoder)
