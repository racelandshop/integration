...
from custom_components.racelandshop.helpers.functions.template import render_template
from custom_components.racelandshop.share import get_racelandshop
from custom_components.racelandshop.const import RACELANDSHOP_GITHUB_API_HEADERS
...
async def get_info_md_content(repository):
    ...
    try:
        ...
        if repository.racelandshop.system.action:
            raise RacelandshopException("::error:: No info file found")
    return ""
...
async def get_repository(session, token, repository_full_name, etag=None):
    ...
    try:
        ...
        raise RacelandshopNotModifiedException(exception) from exception
    except (ValueError, AIOGitHubAPIException, Exception) as exception:
        raise RacelandshopException(exception) from exception
...
async def get_tree(repository, ref):
    ...
    try:
        ...
        raise RacelandshopException(exception)
...
def get_frontend_version():
    ...
    for requirement in manifest.get("requirements", []):
        if requirement.startswith("racelandshop_frontend"):
            frontend = requirement.split("==")[1]
            break
    return frontend


def read_racelandshop_manifest():
    """Reads the RACELANDSHOP manifest file and returns the contents."""
    racelandshop = get_racelandshop()
    content = {}
    with open(
        f"{racelandshop.core.config_path}/custom_components/racelandshop/manifest.json"
    ) as manifest:
...
async def get_integration_manifest(repository):
    ...
    if not manifest_path in [x.full_path for x in repository.tree]:
        raise RacelandshopException(f"No file found '{manifest_path}'")
    try:
        ...
        raise RacelandshopException(f"Could not read manifest.json [{exception}]")
    ...
    try:
        ...
        if repository.racelandshop.system.action:
            if manifest.get("documentation") is None:
                raise RacelandshopException("::error:: manifest.json is missing documentation")
            ...
            #    raise RacelandshopException("The 'issue_tracker' is missing in manifest.json")

        # Set local path
        repository.content.path.local = repository.localpath

    except KeyError as exception:
        raise RacelandshopException(f"Missing expected key {exception} in '{manifest_path}'")
