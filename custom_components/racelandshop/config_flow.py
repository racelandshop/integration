"""Adds config flow for RACELANDSHOP."""
...
from custom_components.racelandshop.const import CLIENT_ID, DOMAIN, MINIMUM_HA_VERSION
from custom_components.racelandshop.helpers.functions.configuration_schema import (
    RELEASE_LIMIT,
    racelandshop_config_option_schema,
)
...
class RacelandshopFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    """Config flow for RACELANDSHOP."""
    ...
    def async_get_options_flow(config_entry):
        return RacelandshopOptionsFlowHandler(config_entry)


class RacelandshopOptionsFlowHandler(config_entries.OptionsFlow):
    """RACELANDSHOP config flow options handler."""

    def __init__(self, config_entry):
        """Initialize RACELANDSHOP options flow."""
    ...
    async def async_step_user(self, user_input=None):
        """Handle a flow initialized by the user."""
        racelandshop: RacelandshopBase = get_racelandshop()
        ...
        if racelandshop.configuration is None:
            return self.async_abort(reason="not_setup")

        if racelandshop.configuration.config_type == "yaml":
            schema = {vol.Optional("not_in_use", default=""): str}
        else:
            schema = racelandshop_config_option_schema(self.config_entry.options)
