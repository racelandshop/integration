...
@pytest.mark.asyncio
async def test_download_content_integration(repository_integration, aresponses, racelandshop):
    aresponses.add(
        "raw.githubusercontent.com",
        aresponses.ANY,
        "get",
    ...
    for path in repository_integration.tree:
        assert os.path.exists(f"{racelandshop.core.config_path}/{path.full_path}")
